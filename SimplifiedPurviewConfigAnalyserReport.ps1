# Optimized script to collect relevant data and output it into a concise JSON file and then Convert it to Excel.
...
Function Get-InformationProtectionSettings {
    ...
    try {
        ...
        try {
            $Collection["GetLabelPolicy"] = Get-LabelPolicy -ErrorAction:SilentlyContinue -WarningVariable +WarnMessage -ForceValidate
        }
        catch {
...
function EvaluateSensitivityLabels {
    ...
    # Predefined sensitivity label names
    #$PredefinedLabels = @("Unofficial", "Official", "Official Sensitive")
...
$Collection | ConvertTo-Json -Depth 3 | Out-File -FilePath $OutputFile -Encoding UTF8
...
function GenerateExcelFromJSON {
    ...
    if ($JsonData.GetLabel) {
        $Columns = @("ImmutableId", "Name", "DisplayName", "Priority", "ParentId", "ParentLabelDisplayName", "IsParent", "Tooltip", "ContentType", "Workload", "IsValid", "CreatedBy", "LastModifiedBy", "WhenCreated", "WhenCreatedUTC", "WhenChangedUTC", "OrganizationId", "LabelActions_Type", "Policy")
        $ExcelSheets += @{
            Name = "GetLabel"
