    ...
    if ($model.SensitivityLabels -and $Collection["GetLabel"] -ne "Error") {
        foreach ($configLabel in $model.SensitivityLabels) {
            ...
            $labelResult = @{
                ...
                Found = $null -ne $discoveredLabel
                Compliant = $false
                Issues = @()
            }

            if ($null -ne $discoveredLabel) {
...
if ($Collection["TenantDetails"] -and $Collection["TenantDetails"] -ne "Error" -and $Collection["TenantDetails"] -ne "No Active Records") {
    $TenantDetails = @{
